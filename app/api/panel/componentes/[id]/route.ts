  ...
  try {
    ...
    const [updated] =
      Object.keys(patch).length > 0
        ? await db.update(component).set(patch).where(eq(component.id, id)).returning()
        : await db.select().from(component).where(eq(component.id, id)).limit(1);
...
// habilitados (FK con onDelete: cascade).
